...
olympics_df = pd.read_excel("olympics.xlsx")
...
st.markdown('<p class="big-font"> Here is a display of the olympics dataframe: </p>', unsafe_allow_html=True)
st.dataframe(olympics_df)

if st.button("Click here to see a condensed Dataframe"):
    st.dataframe(olympics_df[['Medal']].head())
    st.write("This dataframe shows just the columns of .....")
...
n_bins = 10
fig, ax = plt.subplots()
ax.hist(fish_df['Medal'], edgecolor = "black")
